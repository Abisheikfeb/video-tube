...
const VideoDetails = () => {
  const [video, setVideo] = useState({}); // Default state as an empty object
  const [relatedVideos, setRelatedVideos] = useState({ contents: [] }); // Default state with empty contents array
  const { id } = useParams();
          ...
          <div className="flex justify-between flex-col md:flex-row mt-4">
            ...
            <div className="flex text-white mt-4 md:mt-0">
              <div className="flex items-center justify-center h-11 px-6 rounded-3xl bg-white/[0.15]">
                ...
                <span>{`${abbreviateNumber(video?.stats?.likes || 0, 2)} Likes`}</span>
              </div>
